...
// extern int epson_page (const char *pathname);
// extern int countpage (const char *pathname);
...
int    processing(const int *sockptr);
int    open_socket(int port, int (*function)(const int*));
...
static char *wakeup_prog = NULL;
static char *cntpglib = NULL;
...
int main(int argc, char *argv[]) {
	...
	if ((sysconfig = initial_sysconf_module (
		...
		wakeup_prog = sysconfig->getstr ("wake-up-prog");
		cntpglib = sysconfig->getstr ("count-page-lib");
	}
...
static int add_print_job (char *uname, char *jname,
	...
	if (cntpglib != NULL) {
		...
		if ((libp = dlopen (cntpglib, RTLD_LAZY)) != NULL) {
			pagecnt = dlsym (libp, "countpage");

			if ((err = dlerror ()) == NULL) {
				page = pagecnt (file);
			} else {
				do_log (LOG_NOTIFY, "dlsym: %s\n", err);
			}

			dlclose (cntpglib);
		} else {
			do_log (LOG_NOTIFY, "dlopen(%s): %s\n",
					page, file);
		}

		do_log (LOG_NOTIFY, "Precount: %d page(s) [%s]", page, file);
	}
	...
	mydb->query (mydb,
		...
		"qj_state=1,precnt=%d,fsize=%d WHERE sn=%d AND qj_state=0",
		user, jobn, hnbf, file, page, dfsize, job_id);
...
static int readfile (char *fname, const int sockfd, const int ctrlfile,
							const int fsize) {
	int		i, ln = 0, lnp = 0, len = 0;
	int		fd = -1;
	// time_t	start_time;
	char		buffer[16384];
	int		tsize = 0;
	...
	*/
	...
	if (job_id != 0) {
		if (ctrlfile) {
			sprintf (fname, "%s/p%d-%09d.txt",
						spooldir, queueid, job_id);
		} else {
			sprintf (fname, "%s/p%d-%09d.prn",
						spooldir, queueid, job_id);
		}
	...
	// fprintf (stderr, "job id = %d, [%s]\n", job_id, fname);

	if (ctrlfile) {
		int	lenx;

		for (lenx = fsize; lenx > 0; lenx -= len) {
			if (lenx < sizeof buffer) {
				len = lenx;
			} else {
				len = sizeof buffer;
			}

			// fprintf (stderr, "Rctl=%d\n", len);

			if ((len = read (sockfd, buffer, len)) > 0) {
				if (fd < 0) continue;

				for (i = ln = 0, lnp = len;
						lnp > 0; i += ln, lnp -= i) {
					if ((ln = write (fd, &buffer[i],
								lnp)) <= 0) {
						break;
					}
				}
	...
	*/
	...
	if (! ctrlfile) {
		double	elapsed_time = timer->elapsed (timer);
		double	kbps;

		if (elapsed_time > 0.0) {
			kbps  = (double) tsize * 1000. / elapsed_time / 1024.;
...
int Receive_job_subcommand (const int sockfd, const char *queue) {
	...
	while ((len = read (sockfd, buffer, BUFFSIZE)) > 0) {
		...
		case 3: /* Receive data file      03  Count  SP  Name  LF  */
			...
			dfsize = readfile (datafile, sockfd, 0, dcount);
			send_ok (sockfd);
			break;
...
int processing (const int *sockptr) {
	...
	int			rc = 0;
	char			**result;
	const char		*no_entry = "no entries\n";
	...
	if (restrict_host) {
		mydb->query (mydb,
			"SELECT sn,yn,priv FROM hosts_allow WHERE ip = '%s'",
			remoteip);

		if ((result = mydb->fetch (mydb)) != NULL) {
			if (result[1][0] == '1') found = 1;
			if (result[2][0] == '1') priv  = 1;
		} else {
			mydb->query (mydb,
				"SELECT sn,yn,priv FROM hosts_allow WHERE ip="
				"'%d.%d.%d.'",
				ipx[0], ipx[1], ipx[2]);
			...
			if ((result = mydb->fetch (mydb)) != NULL) {
				...
				mydb->query (mydb,
					"SELECT sn,yn,priv FROM hosts_allow "
					"WHERE ip="
					"'%d.%d.'",
					ipx[0], ipx[1]);
...
//  if (remoteport >= 1024 || ! found) {
	...
	case 2: /* Receive a printer job   */
		...
		mydb->query (mydb,
			"SELECT qid,ntdpp,psvc FROM pqueue WHERE qname = '%s'"
		...
		if ((result = mydb->fetch (mydb)) != NULL) {
			...
			if ((priv == 0) && (psvc == 1)) {
				...
				do_log (LOG_FORDEBUG,
					"Receive a printer job (%s) "
					"[priv=%d,cost=%s]",
					buffer, priv, result[1]);
			} else {
				do_log (LOG_FORDEBUG,
					"Receive a printer job (%s) "
					"[psvc=%d]", buffer, psvc);
			...
			do_log (LOG_FORDEBUG,
				"Receive a printer job (%s) [Queue Not found]",
				buffer);
		...
		send_ok (sockfd);
		rc = Receive_job_subcommand (sockfd, buffer);
	...
	case 3: /* Send queue stat (short) */
		...
		write (sockfd, no_entry, sizeof no_entry);
		// send_ok (sockfd);
		break;
    ...
    do {
		...
		*/
            ...
            case  0: /* child process */
                setgid (gid);
                setuid (uid);
                if (function (&msgsock) > 0) {
		    if (wakeup_prog != 0) {
		        execl (wakeup_prog, wakeup_prog, NULL);
		    }
		}
                exit (0);
                break;
...
/*  struct sockaddr_in  clientAddr;  */
    ...
    if (do_not_resolve) return inet_ntoa(clientAddr->sin_addr);
