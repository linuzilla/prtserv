...
static short	use_precnt_number = 0;
#endif
...
static struct pjl_state_t	pjl_table[] = {
	...
	{ 1, 0, 1, 5, 60, NULL, "@PJL INFO PAGECOUNT" },
	{ 3, 5, 2, 0, 61, NULL, "^[0-9]+$" },
	{ 2, 0, 0, 0,  5, NULL, "%%[" },
	{ 3, 1, 0, 0,  6, NULL, "ID=" },
...
static void pjl_receieve (char *str) {
	...
	for (i = match = 0; i < len; i++) {
		...
		if (ptr->p_or_f == 2) {
			...
			if (strcasecmp (str, ptr->string) == 0) {
				match = 1;
			}
		} else {
			j = strlen (ptr->string);

			if (strncasecmp (str, ptr->string, j) == 0) {
				match = 1;
			}
		}
		...
		case 40:	// NAME=
			...
			break;
		case 60: // @PJL INFO PAGECOUNT
			fprintf (stdxx, "INFO PAGECOUNT\n");
			break;
		case 61:
			fprintf (stdxx, "[INFO PAGECOUNT=%s]\n", str);
			break;
		default:
			fprintf (stdxx, "(%s) V=%d\n", str, ptr->value);
			break;
...
static int printer_status (void) {
	...
	if (printer_ready != NULL) {
		...
		for (i = 0; i < num_p_ready; i++) {
			len2 = strlen (printer_ready[i]);

			if (len - 2 < len2) continue;

			if (strncasecmp (printer_ready[i],
						&ptr[1], len2) == 0) {
				return 0;
			}
		}
...
int main (int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv,  "c:df:h:l:Pp:t:vw")) != EOF) {
		switch (c) {
		case 'c':
			cfgfile = optarg;
			break;
...
*/
	...
	if ((sysconfig = initial_sysconf_module (
		...
		if ((i = sysconfig->getint ("use-pre-count-page")) > 0) {
			use_precnt_number = i;
		}
	} else {
		exit (0);
	...
	if ((mydb = new_dbmysql ()) == NULL) {
		...
		for (i = rc = 0; i < 3; i++) {
			mydb->query (mydb,
				"SELECT spooler.*,pqjob.*,pqueue.* "
				"FROM spooler "
				"LEFT JOIN pqjob USING (sn) "
				"LEFT JOIN pqueue USING (qid) "
				"LEFT JOIN pj2print ON "
				"spooler.account = pj2print.account "
				"WHERE pqjob.qid = %d AND spooler.spst = 0 "
				"AND pj2print.account IS NULL "
				"ORDER BY spooler.ptime "
				"LIMIT 1",
				my_queue_id
			);
		...
		precnt = atoi (obji->get (xobj, "precnt"));
		...
		if (debug_flag) {
			...
			if ((ptr = sysconfig->getstr ("logdir")) != NULL) {
				...
				if ((stdxx = fopen (filepath, "w")) == NULL) {
					stdxx = stderr;
				} else {
		...
		if (ntdpp > 0) {
			...
			if ((use_precnt_number &&
					(user_quota < precnt * ntdpp)) ||
					(user_quota < ntdpp)) {
				fprintf (stdxx,
					"Quota: %d, NT$/Page: %d, Require: %d\n"
					, user_quota, ntdpp, precnt * ntdpp);

				mydb->query (mydb,
					"INSERT INTO uselog "
					"(account,qid,pprint,unituse,"
				...
				mydb->query (mydb,
					"DELETE FROM pqjob WHERE sn = %d",
					pq_sn
				);

				mydb->query (mydb,
					"DELETE FROM spooler "
					"WHERE serial_no = %d", spo_sn
				);
...

	{
		...
		sprintf (jobname, "hpunix-%03d", getpid () % 1000);
	}
...
#  endif
...
#if ENABLE_MYSQL_SUPPORT == 1
	{
		...
		for (i = ok = 0; i < 10; i++) {
			if ((sockfd = tcp_connect_to (
						hp_printer_ip, port)) >= 0) {
				ok = 1;
				break;
			}
			sleep (10);
		...
		if (! ok) {
			// Automatic turn-off the printer ....
			mydb->query (mydb,
				"UPDATE printer SET pstate=0 WHERE pid=%d",
				my_printer_id
			);

			mydb->query (mydb,
				"REPLACE INTO prn_alert "
				"(pid,mtype,message,etime) VALUES "
				"(%d,1,'turn off', NOW())",
				my_printer_id
			);
	...
	if (fd < 0) {
		...
		write_pjl  (sockfd, "@PJL INFO PAGECOUNT");
		write_pjl  (sockfd, "@PJL INFO STATUS");
		// Get return
		...
		if (endpage <= 0) {
			...
			write_pjl  (sockfd, "@PJL SET PAGES=%d", endpage);
		}
		// Get return
		write_pjl  (sockfd, "@PJL SET COPIES = 1");

		if (pdf_flag == 1) {
			write_pjl  (sockfd, "@PJL ENTER LANGUAGE=PDF");
		} else {
...
#if ENABLE_MYSQL_SUPPORT == 1
	mydb->query (mydb,
		"UPDATE pquota SET pquota=pquota-%d,puse=puse+%d,lastuse=NOW() "
		"WHERE account='%s'",
		number_of_pages * ntdpp,
		number_of_pages * ntdpp,
		user_acct
	);
	...
	if (file_to_print != NULL) {
		if ((! user_canceled) && (precnt > 0) &&
						(precnt != number_of_pages)) {
			char	*newname = strdup (file_to_print);
			int	len;
			...
			if (rename (file_to_print, newname) != 0) {
				unlink (file_to_print);
			}
